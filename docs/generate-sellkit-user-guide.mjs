...
const screenshots = [
  {
    ...
    caption: "Use the approved account and land back on /sellkit after authentication.",
  },
  {
    file: "02-onboarding.png",
    title: "Complete Mariah onboarding",
    caption: "Answer the five setup questions that shape account fit, buyer titles, signals, and email voice.",
  },
  {
    file: "03-provider-stack-review-queue.png",
    title: "Review provider stages and queue",
    caption: "SellKit shows how Exa, Parallel, Apollo, and Clay contribute before Mariah reviews targets.",
  },
  {
    file: "04-committee-and-evidence.png",
    title: "Inspect committee and evidence",
    caption: "Each target includes signal evidence, a buying committee map, and score factors with source links.",
  },
  {
    file: "05-approve-manual-handoff.png",
    title: "Approve the draft for manual handoff",
    caption: "Approve only after reviewing the draft. SellKit prepares copy, .eml, and CSV export, but does not send.",
  },
];
...
async function captureScreenshots() {
  ...
  try {
    ...
    await onboardingPage.waitForSelector("#sellkit-offerLine", { timeout: 10000 });
    ...
    await providerPage.screenshot({ path: join(screenshotDir, "03-provider-stack-review-queue.png") });
    await providerPage.close();

    const evidencePage = await createAuthedPage(browser);
    await evidencePage.evaluate(() => {
      [...document.querySelectorAll("h3")]
        .find((heading) => heading.textContent?.includes("Buying Committee Map"))
        ?.scrollIntoView({ block: "start" });
    });
    await new Promise((resolveDelay) => setTimeout(resolveDelay, 350));
    await evidencePage.screenshot({ path: join(screenshotDir, "04-committee-and-evidence.png") });
    await evidencePage.close();

    const approvalPage = await createAuthedPage(browser);
    await clickButtonByText(approvalPage, "Approve Draft");
    await new Promise((resolveDelay) => setTimeout(resolveDelay, 500));
    await approvalPage.screenshot({ path: join(screenshotDir, "05-approve-manual-handoff.png") });
...
function screenshotFigure(shot) {
  return `
    <figure>
      <img src="../output/guide-screenshots/sellkit/${escapeHtml(shot.file)}" alt="${escapeHtml(shot.title)}" />
...
<body>
  <main>
    <section class="cover">
      ...
      <p>A short operating guide for Mariah Rubino to sign in, complete setup, review signal-backed targets, approve manual outreach drafts, and export rows without automatic sending.</p>
      <div class="meta">
    ...
    <section class="card">
      ...
      <p>SellKit helps Mariah review account and buyer recommendations before outreach. It combines account discovery, buyer discovery, signal evidence, buying committee research, scoring, and draft copy into one review queue.</p>
      <div class="callout">
        <strong>No automatic send.</strong> SellKit prepares drafts and exports. Mariah reviews and approves before anything is copied, downloaded, or imported elsewhere.
      </div>
    </section>
    ...
    <section class="card">
      ...
      <p class="section-lede">Click <code>Edit Onboarding</code> and answer the five setup questions. The goal is not a perfect spec. The goal is enough context for SellKit to recognize the right big-tech account, the right executive buyer, the right trigger, and the right expert-services angle.</p>
    ...
    <section class="card">
      ...
      <ol>
        ...
        <li><strong>Check provider stages:</strong> confirm the provider architecture matches the workflow: Exa for discovery, Parallel for fast lookup and preview, Exa Agent for dossiers, Apollo and Clay for selective enrichment.</li>
        ...
        <li><strong>Read evidence:</strong> open source links where needed. The score should be explainable from visible evidence.</li>
        <li><strong>Check the buying committee:</strong> identify primary buyer, technical co-sponsor, and budget or ops sponsor.</li>
        <li><strong>Approve only when ready:</strong> approve the draft after reviewing copy. Then copy/download the manual email and export CRM CSV if needed.</li>
      </ol>
    ...
    <section class="card">
      ...
      <p>If SellKit sends you to the sign-in page, use the approved account and return to <code>/sellkit</code>.</p>
      ...
      <table class="matrix">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td data-label="Guardrail">Use after fit and signal scoring. Batch in groups of 10. Avoid phone and personal email by default.</td>
          </tr>
          <tr>
    ...
    <section class="card">
      ...
      <ul>
        ...
        <li><strong>CRM export is not ready:</strong> approve at least one target.</li>
        <li><strong>Draft sounds wrong:</strong> update the onboarding email voice field with a real example.</li>
